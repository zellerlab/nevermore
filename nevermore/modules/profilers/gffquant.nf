...
process stream_gffquant {
	...
	output:
        ...
        tuple val(sample), path("alignments/${sample}/*.bam"), emit: alignments

	script:
			def gq_output = "-o profiles/${sample}/${sample}"

			def gq_params = "-m ${params.gq_mode} --ambig_mode ${params.gq_ambig_mode} --keep_alignment_file ${sample}.sam"
