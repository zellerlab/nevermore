...
process stream_gffquant {
	...
	output:
        ...
        tuple val(sample), path("alignments/${sample}/*.bam"), emit: alignments, optional: true
	...
	script:
			...
			def save_alignments = (params.keep_alignments != null && params.keep_alignments != false) ? "cat ${sample}_merged.sam | awk -F'\t' '!/^@/ {for (i=1; i<=NF; i++) if (i==10 || i==11) \$i="*"} 1' OFS='\t' | samtools view -F 4 -buSh -o alignments/${sample}/${sample}.bam" : ""
			"""
			set -e -o pipefail
